...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        cv2.imwrite(os.path.join(depth_path, '{0:05d}'.format(idx) + ".png"), image_colored)
...
def render_circular_video(model_path, iteration, views, gaussians, pipeline, background, args, radius=0.5, n_frames=240):
 ...
 #        torchvision.utils.save_image(rendering, os.path.join(render_path, '{0:05d}'.format(idx) + ".png"))
 #      im = rendering.cpu().numpy().transpose(1, 2, 0 )
 #      frames.append( (im * 255).astype(np.uint8) )
 #  imageio.mimwrite(os.path.join(render_path,'renders_circle_video.mp4'), frames, fps=args.fps, macro_block_size=16, quality=8)
...
def render_spiral_video(model_path, iteration, views, gaussians, pipeline, background, args):
    ...
    import subprocess
    process = subprocess.run( ['ffmpeg','-y', '-framerate', '30', '-i', render_path+'/%05d.png', '-vf','pad=ceil(iw/2)*2:ceil(ih/2)*2','-c:v', 'libx264','-r','30','-pix_fmt', 'yuv420p',  \
                               render_path+'/renders_spiral_video.mp4'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    print(process.stdout.decode())
    print(process.stderr.decode())
...
#       im = rendering.cpu().numpy().transpose(1, 2, 0 )
#       frames.append( (im * 255).astype(np.uint8) )
#   imageio.mimwrite(os.path.join(render_path,'renders_sphericalsample_video.mp4'), frames, fps=args.fps, macro_block_size=16, quality=9)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--total_frame", default=600, type=int)
